...
var generateOutputTransactions = function (poolRecipient, recipients, rpcData, network, poolOptions) {
    ...
    // -------------------------------------------------------
    // MWC mandatory developer fee (daemon-defined)
    // -------------------------------------------------------
    var developerReward = 0;
    var developerScript = null;

    if (rpcData.developer && rpcData.developer.amount > 0) {
        developerReward = rpcData.developer.amount;
        developerScript = Buffer.from(rpcData.developer.script, 'hex');
    ...
    for (var i = 0; i < recipients.length; i++) {
        ...
        if (recipients[i].percent == 0) {
            ...
            recipientReward = Math.floor(recipients[i].percent * fullReward);
        }
    ...
    // -------------------------------------------------------
    // Append developer output (AFTER other special payees)
    // -------------------------------------------------------
    if (developerReward > 0 && developerScript) {
        txOutputBuffers.push(Buffer.concat([
