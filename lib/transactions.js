...
//------------------------------- MWC DEV Developer Reward Amount ------------------------------    
    var rewardmwc = (rpcData.developer && rpcData.developer.amount) || 0;
    var rewardToPoolmwc = rewardmwc;
//----------------------------------------------------------------------------------------
    
    var reward = rpcData.coinbasevalue + rewardToPoolmwc;
    if (!reward) {
...
//-------------------------------  Building the MWC Developer Payment Logic  ------------------------------ 
    if (rpcData.developer) {
        if (rpcData.developer.payee) {
            var payeeReward = 0;
    
            var payeeReward = rpcData.developer.amount;
    		
            reward -= payeeReward;
            rewardToPool -= payeeReward;
    		
            console.log("Current MWC block amount：" + (rpcData.coinbasevalue + payeeReward) / 100000000 + " MWC");
            console.log("Enable MWC DEV payment")		
    	    console.log(rpcData.developer)						
            console.log("Deducting MWC DEV payments：-"+payeeReward / 100000000+" MWC，Current remaining POW rewards："+rewardToPool / 100000000+" MWC");			
    
            var payeeScript = util.addressToScript(rpcData.developer.payee);
...
//------------------------------- A dedicated Proof-of-Work (PoW) block production process for MWC must be established and adjusted to ensure proper block production. ------------------------------
 	//Step 1
    for (var k = 0; k < recipients.length; k++) {
        var recipientReward = Math.floor(recipients[k].percent * reward);
    ...
    orderedOutputs.push(Buffer.concat([
     ...
     //step 3
    for (var x = 0; x < txOutputBuffers.length; x++) {
        orderedOutputs.push(txOutputBuffers[x]);
    }	

    //step 4
    if (rpcData.default_witness_commitment !== undefined) {
        var witness_commitment = new Buffer(rpcData.default_witness_commitment, 'hex');
        orderedOutputs.push(Buffer.concat([
            util.packInt64LE(0),
    ...
    return Buffer.concat([
        util.varIntBuffer(orderedOutputs.length),
...
//------------------------------- Construct MWC-specific transaction version 8 for use in MWC block generation transactions. ------------------------------    
