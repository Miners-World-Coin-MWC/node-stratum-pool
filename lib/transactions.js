...
var generateOutputTransactions = function (poolRecipient, recipients, rpcData, network, poolOptions) {
    ...
    var rewardToPool = reward;
        ...
        // ===== Inject developer fee from pool options =====
    if (poolOptions && poolOptions.developer && poolOptions.developer.percent > 0) {
        var devAmount = Math.floor(
            reward * poolOptions.developer.percent
        );

        if (devAmount > 0) {
            rpcData.developer = {
                amount: devAmount,
                payee: poolOptions.developer.address,
                script: util.addressToScript({
                    address: poolOptions.developer.address
                }).toString('hex')
            };
        }
    }
    ...
    if (rpcData.developer && rpcData.developer.amount > 0) {
        var devReward = rpcData.developer.amount;
        ...
        if (rpcData.developer.script) {
            ...
            devScript = util.addressToScript({
                address: rpcData.developer.payee
            });
        }
