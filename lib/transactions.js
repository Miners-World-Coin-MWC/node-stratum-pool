...
//-------------------------------  二.构建 MWC Developer 支付付款逻辑  ------------------------------ 
    if (rpcData.developer) {
        if (rpcData.developer.payee) {
            var payeeReward = 0;
    
            payeeReward = rpcData.developer.amount;
    		
            reward -= payeeReward;
            rewardToPool -= payeeReward;
    		
            console.log("当前 MWC 爆块金额：" + (rpcData.coinbasevalue + payeeReward) / 100000000 + " MWC");
            console.log("启用 MWC DEV 付款")		
    	    console.log(rpcData.developer)						
            console.log("扣除 MWC DEV 付款：-"+payeeReward / 100000000+" MWC，目前剩余 POW 奖励："+rewardToPool / 100000000+" MWC");			
...
//------------------------------- 三.构建 MWC 专用 POW 出块流程，必须调整才能正常出块 ------------------------------
 	//第1步
